...
class FreeCADKernelAdapter:
    ...
    def _create_sketch(self, document: Any, action: Action) -> None:
        ...
        self._ensure_property(obj, "App::PropertyString", "CadCopilotGeometryMap")
        obj.CadCopilotGeometryMap = "{}"
        if plane == "XY":
    ...
    def _add_sketch_geometry(self, document: Any, action: Action) -> None:
        ...
        sketch.CadCopilotGeometryMap = json.dumps(mapping, sort_keys=True)
    ...
    def _feature_id(obj: Any) -> str:
        value = getattr(obj, "CadCopilotId", None)
        return value if isinstance(value, str) and value else str(obj.Name)

    @staticmethod
    def _feature_kind(obj: Any) -> str:
        value = getattr(obj, "CadCopilotKind", None)
        if isinstance(value, str) and value:
            return value
        return str(getattr(obj, "TypeId", "unknown"))

    def _tag(self, obj: Any, feature_id: str, feature_kind: str) -> None:
        self._ensure_property(obj, "App::PropertyString", "CadCopilotId")
        self._ensure_property(obj, "App::PropertyString", "CadCopilotKind")
        obj.CadCopilotId = feature_id
        obj.CadCopilotKind = feature_kind
    ...
    def _geometry_map(sketch: Any) -> dict[str, int]:
        raw = getattr(sketch, "CadCopilotGeometryMap", "{}") or "{}"
        try:
